...
const navNameArr = {
    home: { name: "后台管理", icon: "SettingFilled" },
    order: { name: "订单管理", icon: "AccountBookFilled" },
    user: { name: "用户管理", icon: "IdcardFilled" },
};
let routeItem = {};
files.keys().forEach((key) => {
    ...
    if (typeof route === "function") {
        ...
        if (!routeItem[pathArr[0]]) {
            routeItem[pathArr[0]] = {
                nav: pathArr[0],
                navName: navNameArr[pathArr[0]].name,
                icon: navNameArr[pathArr[0]].icon,
                com: [{ path: path, name: name, component: route, auth: true }],
            };
        } else {
            routeItem[pathArr[0]].com.push({
                path: path,
                name: name,
                component: route,
                auth: true,
